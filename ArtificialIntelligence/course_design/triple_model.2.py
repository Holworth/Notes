...
def train(input, output, model, hidden_dim, epochs):
    ...
    model = LSTMTagger(INPUT_DIM, hidden_dim, OUTPUT_DIM).cuda()
    # model = RNN(INPUT_DIM, hidden_dim, OUTPUT_DIM).cuda()
    ...
    for epoch in range(epochs): # 我们要训练300次，可以根据任务量的大小酌情修改次数。
        ...
        for sentence in data_list:
            ...
            for each_case in sentence['testset']:
                ...
                #     print(sentence_in[i])
                #     output, hidden = model(torch.tensor(sentence_in[i], dtype=torch.float).cuda())
                # tag_scores=[output]

                # 计算损失，反向传递梯度及更新模型参数
                # print(tag_scores[-1].view(1,-1))
                # print(torch.autograd.Variable(torch.tensor(tags)).view(1,-1))
                # loss = loss_function(tag_scores[-1].view(1,-1), torch.autograd.Variable(torch.tensor(tags)).view(1,-1))
                # loss = loss_function(tag_scores[-1], torch.autograd.Variable(torch.tensor(tags)))
                if tag_scores[-1][1]>tag_scores[-1][0]:
                    ...
                    if(not is_triple):
                        right=right+1
                cases=cases+1
                loss = loss_function(tag_scores[-1].view(1,-1).cuda(), torch.autograd.Variable(torch.tensor([int(is_triple)])).cuda())
                loss.backward()
                optimizer.step()
