...
//Disjoint set: used only for int set; or set marked with int;
class DisjointSet
{
public:
  //use negative numbers to represent the number of elements the set contains, positive number means its parent;
  ...
  int &operator[](int i)
  {
    //it is highly recommend for you to use find() and merge() instead
    return data_[i];
  }
  ...
  bool unique()
  {
    //can only be used when you use find and merge to deal with the disjoint set;
  ...
  int merge(int a, int b)
  {
    if (find(a) == find(b))
      return 0;
    if (data_[a] < data_[b]) //a has more element than b
    {
      data_[a] += data_[b];
      data_[b] = a;
    }
