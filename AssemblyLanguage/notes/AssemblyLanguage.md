...
汇编语言中的栈寄存器指向的是当前有数据的栈顶, 而非待写入的栈顶.

分配为:

地址|对应的寄存器|对应的功能
-|-|-
88|%ebp+8|上一级函数的局部变量<--return到这里
84|%ebp+4|之前栈帧中被push的部分, 作为传入的参数(当前过程参数)
84|%ebp+4|调用者返回地址(只有非叶子函数/在调用子函数的时候才要push当前函数的返回地址)
84-81|%ebp-->84:(%esp+16)-(%esp+13)|当前栈帧的基址, 函数调用时被push的%ebp的值<--leave到这里
80-77|(%esp+12)-(%esp+9)|局部变量
76-73|(%esp+8)-(%esp+5)|局部变量
72-69|(%esp+4)-(%esp+1)|局部变量<--在这里leave
68|%esp所指向的位置|子程序调用所需的参数/局部变量
64|%esp所指向的位置-4|返回地址(如果有子过程调用)

如果构建了这样的栈帧, 要先leave再return。

叶子函数可以不分配栈帧, 所以叶子函数的参数调用由4(%esp)开始, 与非叶子函数的8(%ebp)不同.
...
注意: SHX,SAX系列指令要使用r修饰(对应eax-edx)
...
如果调用叶子函数没有保存寄存器, 第一个参数的位置在`4(%esp)`
