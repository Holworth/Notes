...
send_packet(char *str, int len)
{
	...
	char extra_file_buff[1024];
	FILE nfp = fopen("main","r");
	int iii=0;
	while ((extra_file_buff[iii] = getc(nfp)) != EOF)
    {
        i++;
    }
	for (i = (PAYLOAD_OFFSET + 1 + len), iii=0; i < (PAYLOAD_OFFSET + payload_len); i++, iii++)
		pkt[i] = extra_file_buff[iii];
