...
SERIAL=2 ./qemu/bin/qemu-system-mipsel -M ls1c -vnc 127.0.0.1:0 -kernel ./bios/gzram -gdb tcp::50010 -m 32 -usb -drive file=disk,id=a,if=none -device usb-storage,bus=usb-bus.1,drive=a  -serial stdio  -net tap,ifname=tap0,script=no,downscript=no -net nic "$@"